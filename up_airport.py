    ...
    def checkin(url, email, password):
        ...
        try:
           ...
           # rest = re.findall(r'\["Unused_Traffic", "(.*?)"]', response.text)[0]
            ...
            count_arr=soup.find_all(attrs={'class': 'counter'})
            last_day=soup.find(attrs={'class': 'breadcrumb-item active','aria-current': 'page'})
            msg = (
                url
                + "\n- 今日签到信息："
                + str(msg)
               ...
               # + str(level)
                + "\n- 会员时长："
                + str(count_arr[0].string)
                + "天"
                + "\n"
                + str(last_day.contents[0]).strip()
                + "\n- 剩余流量："
                + str(count_arr[1].string)
                + "GB"
            )
            return msg
        except Exception:
